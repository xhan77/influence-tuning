...
def main():
    ...
    parser.add_argument("--no_epoch_checkpoint_saving",
                        action='store_true',
                        help="Whether not to save model for each epoch")
    parser.add_argument("--adv_lambda_if_applying_dann",
                        type=float,
                        default=0,
                        help="If using Domain Adversarial NN, set the lambda value")
    ...
    if 'synth' in args.mode:
        ...
        label_list = processor.get_labels()
        num_labels = len(label_list)
    elif 'anli' in args.mode:
        processor = NLIProcessor_ANLI()
        label_list = processor.get_labels()
        num_labels = len(label_list)
    else:
        processor = SAProcessor_TextAttack()
        label_list = processor.get_labels()
        num_labels = len(label_list)
    ...
    if args.do_train:
        ...
        elif args.mode == 'anli_only':
            train_examples = processor.get_train_examples(args.data_dir)
            num_train_optimization_steps = int(
                len(train_examples) / args.train_batch_size) * args.num_train_epochs
        elif args.mode == 'train_subset':
            train_examples = processor.get_train_orig_examples(args.data_dir)
            num_train_optimization_steps = int(
                len(train_examples) / args.train_batch_size) * args.num_train_epochs
        elif args.mode:
            train_examples = processor.get_train_examples(args.data_dir)
    ...
    if args.trained_model_dir: # load in fine-tuned (with cloze-style LM objective) model
        ...
        previous_state_dict.update(distant_state_dict) # the final layers of previous model and distant model must have different attribute names!
        if args.adv_lambda_if_applying_dann > 0:
            if args.mode == 'synth' or args.mode == 'msgs':
                model = MyBertPlusAdvForSequenceClassification.from_pretrained(args.trained_model_dir, state_dict=previous_state_dict,
                                                                               num_labels=num_labels, num_confounds=2,
                                                                               balancing_lambda=args.adv_lambda_if_applying_dann) # hardcode for now
            else:
        ...
        if args.adv_lambda_if_applying_dann > 0:
            if args.mode == 'synth' or args.mode == 'msgs':
                model = MyBertPlusAdvForSequenceClassification.from_pretrained(args.bert_model, cache_dir=cache_dir,
                                                                               num_labels=num_labels, num_confounds=2,
                                                                               balancing_lambda=args.adv_lambda_if_applying_dann) # hardcode for now
            else:
                raise ValueError("N/A")
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if args.adv_lambda_if_applying_dann > 0:
                    if args.mode == 'synth' or args.mode == 'msgs':
                        confound_ids = note[:, 1] # Han: in the toy data, position 0 is core attrib, position 1 is spur attrib
                    else:
                        raise ValueError("N/A")
                    agg_loss, loss, adv_loss = model(input_ids, segment_ids, input_mask, label_ids, confound_ids)
                    agg_loss.backward()
                else:
                ...
                optimizer.zero_grad()
                global_step += 1
                if args.adv_lambda_if_applying_dann > 0: # debug
                    epoch_loss.append(adv_loss.item())
                else:
    ...
    if args.do_test:
        if args.mode == 'synth' or args.mode == 'msgs':
            ...
            test_examples_seq = [processor.get_train_examples(args.data_dir),
                                 processor.get_test_examples(args.data_dir)]
            idx2setname = {0: 'train_set', 1: 'test_set'}
        elif args.mode == "orig_train":
            test_examples_seq = [processor.get_train_examples(args.data_dir),
                                 processor.get_test_examples(args.data_dir),
                                 processor.get_train_orig_examples(args.data_dir),
                                 processor.get_test_orig_examples(args.data_dir)]
            idx2setname = {0: 'train_full', 1: 'test_full', 2: 'train_subset', 3: 'test_subset'}
        elif args.mode == "test_adv":
            test_examples_seq = [processor.get_train_adv_examples(args.data_dir),
                                 processor.get_test_adv_examples(args.data_dir)]
            idx2setname = {0: 'train_adv', 1: 'test_adv'}
        elif args.mode == "all":
            test_examples_seq = [processor.get_train_orig_examples(args.data_dir),
                                 processor.get_test_orig_examples(args.data_dir),
                                 processor.get_train_adv_examples(args.data_dir),
                                 processor.get_test_adv_examples(args.data_dir)]
            idx2setname = {0: 'train_subset', 1: 'test_subset', 2: 'train_adv', 3: 'test_adv'}
        else:
            raise ValueError("Check your args.mode")
